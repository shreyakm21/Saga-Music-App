...
// Route to fetch tracks from Jamendo API
/*app.get('/api/jamendo-tracks', async (req, res) => {
  try {
    const jamendoApiUrl = 'https://api.jamendo.com/v3.0/tracks';
    const clientId = '09c35c07'; // Your client ID
    const url = `${jamendoApiUrl}?client_id=${clientId}&limit=5&order=popularity_week`;

    // Make the request to the Jamendo API
    const response = await fetch(url);
    if (!response.ok) {
      return res.status(500).json({ message: 'Failed to fetch data from Jamendo API' });
    }

    const data = await response.json();
    console.log('Jamendo API Response:', data); // Log the full response

    // Send back the tracks to the frontend
    res.json(data.results); 
  } catch (error) {
    console.error('Error fetching Jamendo tracks:', error);
    res.status(500).json({ message: 'Error fetching Jamendo tracks' });
  }
});*/
app.get('/api/jamendo-tracks', async (req, res) => {
  try {
    ...
    //const url = `${jamendoApiUrl}?client_id=${clientId}&limit=${limit}&offset=${offset}&order=popularity_week`;
    let url = `${jamendoApiUrl}?client_id=${clientId}&limit=${limit}&offset=${offset}&order=popularity_week`;

    if (genre) {
      url += `&tags=${encodeURIComponent(genre)}`; // Add genre to the API request
    }
